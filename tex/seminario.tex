...
\begin{document}
    ...
    \begin{frame}{Ordering relations}
        \begin{definition}[Partial order]
            A \emph{partial order} \(\prec\) on a set \(X\) is a reflexive, antisymmetric,
            transitive relation.
        \end{definition}
        \begin{definition}[Total order]
            A \emph{total order} \(\ge\) on a set \(X\) is a partial order such that, if
            \(a,b\in X\), then either \(a\ge b\) or \(b\ge a\).
        \end{definition}
    \end{frame}
